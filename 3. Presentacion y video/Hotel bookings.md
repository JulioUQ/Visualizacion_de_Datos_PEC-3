...
| ------------------------------ | ---------- | ---------------------------------------------------------------------------------------------------------------------------------- |
| hotel                          | Categórica | Tipo de hotel: City Hotel o Resort Hotel. Representa el establecimiento donde se realizó la reserva.                               |
| is_canceled                    | Entero     | Indica si la reserva fue cancelada (1) o no (0).                                                                                   |
| lead_time                      | Entero     | Días entre la fecha en que se realizó la reserva y la fecha de llegada.                                                            |
| arrival_date_year              | Entero     | Año de llegada del cliente al hotel.                                                                                               |
| arrival_date_month             | Categórica | Mes de llegada (January–December).                                                                                                 |
| arrival_date_week_number       | Entero     | Número de la semana del año correspondiente a la fecha de llegada.                                                                 |
| arrival_date_day_of_month      | Entero     | Día del mes de llegada.                                                                                                            |
| stays_in_weekend_nights        | Entero     | Número de noches de fin de semana (sábado y domingo) reservadas o realmente pasadas en el hotel.                                   |
| stays_in_week_nights           | Entero     | Número de noches entre semana (lunes–viernes) reservadas o realmente pasadas en el hotel.                                          |
| adults                         | Entero     | Número de adultos incluidos en la reserva.                                                                                         |
| children                       | Entero     | Número de niños incluidos en la reserva (suma de niños de pago y no pago).                                                         |
| babies                         | Entero     | Número de bebés en la reserva.                                                                                                     |
| meal                           | Categórica | Tipo de régimen alimenticio: Undefined/SC, BB (Bed & Breakfast), HB, FB.                                                           |
| country                        | Categórica | País de origen del huésped (código ISO 3166-3).                                                                                    |
| market_segment                 | Categórica | Segmento comercial: Online TA, Offline TA/TO, Groups, Direct, Corporate, etc.                                                      |
| distribution_channel           | Categórica | Canal de distribución: TA/TO, Direct, Corporate, GDS, etc.                                                                         |
| is_repeated_guest              | Entero     | Indica si el huésped es repetidor (1) o no (0).                                                                                    |
| previous_cancellations         | Entero     | Número de reservas previas canceladas por el cliente antes de la reserva actual.                                                   |
| previous_bookings_not_canceled | Entero     | Número de reservas previas no canceladas del cliente antes de la reserva actual.                                                   |
| reserved_room_type             | Categórica | Tipo de habitación reservada (código anonimizado).                                                                                 |
| assigned_room_type             | Categórica | Tipo de habitación asignada finalmente. Puede diferir de la reservada por operaciones de hotel u overbooking.                      |
| booking_changes                | Entero     | Número de modificaciones realizadas a la reserva (personas, fechas, habitación o meal plan).                                       |
| deposit_type                   | Categórica | Tipo de depósito: No Deposit, Refundable, Non Refund.                                                                              |
| agent                          | Categórica | ID de la agencia de viajes. “NULL” indica que no intervino ninguna agencia.                                                        |
| company                        | Categórica | ID de la compañía que realizó o pagó la reserva. “NULL” indica no aplicable.                                                       |
| days_in_waiting_list           | Entero     | Días que la reserva permaneció en lista de espera antes de ser confirmada.                                                         |
| customer_type                  | Categórica | Tipo de cliente: Contract, Group, Transient, Transient-party.                                                                      |
| adr                            | Numérico   | Average Daily Rate: ingresos de alojamiento entre número total de noches.                                                          |
| required_car_parking_spaces    | Entero     | Número de plazas de parking solicitadas por el cliente.                                                                            |
| total_of_special_requests      | Entero     | Número de solicitudes especiales realizadas por el cliente (p. ej., camas twin, planta alta).                                      |
| reservation_status             | Categórica | Estado final de la reserva: Canceled, Check-Out, No-Show.                                                                          |
| reservation_status_date        | Fecha      | Fecha en la que se estableció el último estado de la reserva. Permite saber cuándo fue cancelada o cuándo se produjo el check-out. |
